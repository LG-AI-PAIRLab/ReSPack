...
class ESModel(object):
    def __init__(self, net_num, pair_num, node_num):
        rank_weights = np.zeros(shape=net_num)
        maze_weights = np.zeros(shape=(pair_num, node_num))
    ...
    def get_maze_data(self):
        return np.maximum(self.weights[1], 0) * 10.
