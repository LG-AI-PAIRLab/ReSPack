...
class EvolutionStrategy(object):
    ...
    def run(self, iterations):
        ...
        iterator = trange(iterations) if self.verbose else range(iterations)
        ...
        for iteration in iterator:
            ...
            if self.verbose:
                iterator.set_description(f'Reward {reward} (Best {self.best_reward})', refresh=True)

            if self.log_function:
                self.log_function(iteration, self.weights, reward, rewards, time_duration=used_time)
            if self.stop_function and self.stop_function(iteration, None, reward, rewards,
